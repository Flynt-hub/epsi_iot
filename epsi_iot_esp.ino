...
#define ONE_WIRE_PIN 4 // index de la PIN ESP 2

OneWire gOneWire(ONE_WIRE_PIN);
DallasTemperature sensors(&gOneWire);


#define wifi_ssid "DavPhone"
#define wifi_password "testchiante"

#define mqtt_server "192.168.83.244"
...
//r#define DHTTYPE DHT22         // DHT 22  (AM2302)
...
void loop() {
  ...
  if (now - lastMsg > 1000 * 10) {
    ...
    float temperature_simple = sensors.getTempCByIndex(0);
    int lightValue = analogRead(A0);
    Serial.print("luminance relevée par le capteur Grove : ");
    ...
    client.publish(light_topic, String(lightValue).c_str(), true);      //Et la luminescence
  }
  if (now - lastRecu > 100 ) {
    lastRecu = now;
